...
def _parse_result(game: dict, username: str) -> tuple[str, str]:
    ...
    white_res  = game["white"].get("result", "")
    black_res  = game["black"].get("result", "")
    mover_res  = white_res if is_white else black_res
    other_res  = black_res if is_white else white_res
    ...
    if mover_res == "win":
        ...
        reasons = DRAW_REASONS
        key     = mover_res
        result  = "Draw"

    detail = reasons.get(key, f"{result} — {key.title()}")
...
class ChessComClient(BasePlatformClient):
    async def fetch_games(
        ...
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            for archive_url in archives:
                ...
                raw_games = await _fetch_month_games(client, archive_url)
                raw_games = list(reversed(raw_games))

                for game in raw_games:
                    tc_str              = game.get("time_control", "-")
                    tc_readable, speed  = parse_time_control(tc_str)
    ...
    async def fetch_pgn(self, game_id: str, username: str = "") -> str:
        url = f"{BASE_URL}/game/{game_id}/pgn"
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(url, headers=HEADERS)
            if resp.status_code == 404:
                raise ValueError(f"Game '{game_id}' not found on Chess.com.")
            resp.raise_for_status()
            return resp.text'''
from datetime import datetime, timezone
from typing import Optional
